...
# ResNet18, ResNet34 --- residual 
class BasicBlock(nn.Module): 

    expansion = 1 # conv2_x  channel not change  
    
    def __init__(self, in_channel, out_channel,stride = 1, downsample = None): 

        # downsample 对应的是虚线结构： 1*1 卷积层 
...
class ResNet(nn.Module): 
    ...
    def __init__(self, block, blocks_num, num_classes = 100, include_top = True): 
        """
        block: residual block -- bottleneck / basic block 
        ...
        self.layer1  = self._make_layer(block,64,blocks_num[0])  #conv2x   
        self.layer2  = self._make_layer(block,128,blocks_num[1],stride  = 2)
        self.layer3  = self._make_layer(block,256,blocks_num[2],stride  = 2) 
        self.layer4  = self._make_layer(block,516,blocks_num[3],stride  = 2) 
        if self.include_top: 
